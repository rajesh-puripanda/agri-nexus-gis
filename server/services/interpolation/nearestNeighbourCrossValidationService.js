...
// Soil Analysis GIS
//
// Phase 12.7.5  Nearest-Neighbour LOOCV Diagnostics
//
// Scientific purpose:
//
//   Perform leave-one-out cross-validation (LOOCV) for the
//   authoritative nearest-neighbour interpolation engine.
//
// Method:
//
//   For each observation:
//     1. Remove that observation from the training set.
//     2. Use the remaining observations as candidates.
//     3. Find the geographically nearest remaining sample.
//     4. Predict using that sample's observed value.
//     5. Compare prediction with the held-out observation.
//
// Mathematical definition:
//
//   j* = argmin_j d(x_i, x_j),  j != i
//
//   Z_hat_-i(x_i) = Z(x_j*)
//
// Error convention:
...
//
//   - introduce distance weighting
//   - introduce smoothing
//   - introduce a distance-power parameter
//   - average neighbouring observations
//   - define acceptance thresholds
//
// It delegates nearest-neighbour prediction to the
// authoritative nearestNeighbourInterpolation service.
//
// ============================================================

const { interpolateNearestNeighbour } = require("./nearestNeighbourInterpolation");
const { isFiniteNumber } = require("./spatialDistance");
      ...
      errors:
        ...
        [
          interpolationResult?.error ??
            "LOOCV nearest-neighbour interpolation failed.",
        ],
      interpolationError:
        interpolationResult?.error ??
        null,
      nearestSample:
        interpolationResult?.nearestSample ??
        null,
    };
  }
