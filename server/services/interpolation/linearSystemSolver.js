...
//       [ 1ᵀ  0 ] [ μ ] = [ 1 ]
//
//   The Kriging matrix construction is handled separately by
//   krigingMatrix.js.
//
...
function solveLinearSystem(matrix, vector, options = {}) {
  ...
  if (matrixScale === 0) {
    return {
      ...
      errors: [
        "The coefficient matrix contains only zeros.",
      ],
    };
  }
...
function calculateResidual(
  ...
  const residual =
    new Array(matrixValidation.size).fill(0);

  for (
    let row = 0;
    row < matrixValidation.size;
    row += 1
  ) {
